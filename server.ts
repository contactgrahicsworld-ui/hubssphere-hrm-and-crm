...
app.post("/api/calls/save", (req, res) => {
  ...
  lead.journey.push({
    status,
    notes: notes ? `Call Completed: "${notes}" (Duration: ${duration}s)` : `Call Completed (Duration: ${duration}s)`,
  ...
  if (actorId && actorId !== "u-admin") {
    ...
    if (requester) {
      if (requester.role === "admin" || requester.role === "sub-admin") {
        // Sub-admin or admin can see all tasks
...
app.get("/api/payroll/report", (req, res) => {
  ...
  const report = eligibleUsers.map((user: any) => {
      ...
      if (isCompanyHoliday) {
        ...
        if (attRecord) {
          ...
          if (workHours >= 9.0) {
            ...
            presentDays += 0.5;
            const halfDayDed = Number((0.5 * perDaySalary).toFixed(2));
          ...
          if (appLeave) {
            ...
            leaveDays++;
            // First 2 leaves of sanctioned leaves are Paid! Beyond that, unpaid unless admin override is true
            if (approvedLeavesCountSoFar <= 2 || appLeave.payType === "Full Pay" || override.extraLeavePaid) {
              detailDays.push({ 
                date: dateStr, 
                day: d, 
                type: "Leave-Approved-Full", 
                label: `Approved Leave (Paid - count ${approvedLeavesCountSoFar})`, 
              ...
              detailDays.push({ 
                ...
                label: `Approved Leave (Unpaid - Exceeded Limit of 2 leaves)`, 
                deductionFraction: 1.0 
              });
              totalDeductions += perDaySalary;
            }
    ...
    // Performance-based salary cut <= 80%
    ...
    const finalSalary = hasNotJoinedYet ? 0 : Number((finalBasicSalary + incentiveAmount + weekdayOvertimePay + sundayOvertimeEarned).toFixed(2));
